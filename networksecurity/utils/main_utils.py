...
def evaluate_models(X_train, y_train, X_test, y_test, models, param):
    try:
        ...
        for i in range(len(list(models))):
            ...
            # پیدا کردن بهترین پارامترها با استفاده از GridSearchCV
            gs = GridSearchCV(model, para, cv=3)
            gs.fit(X_train, y_train)

            # تنظیم بهترین پارامترها روی مدل و آموزش نهایی
            model.set_params(**gs.best_params_)
            model.fit(X_train, y_train)

            # پیش‌بینی برای داده‌های آموزش و تست
            y_train_pred = model.predict(X_train)
            y_test_pred = model.predict(X_test)

            # محاسبه امتیاز (در اینجا از r2_score استفاده شده)
            train_model_score = r2_score(y_train, y_train_pred)
            test_model_score = r2_score(y_test, y_test_pred)

            # ذخیره امتیاز مدل تست در گزارش
